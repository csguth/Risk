...
std::unique_ptr<lobby> create_lobby(player::id &csguth_id, player::id &other_id) {
    lobby my_lobby;
...
TEST_CASE( "territory without player" ) {
    ...
    REQUIRE(map.owner(territory_id) == player::null_id());
}
...
TEST_CASE( "conquest empty territory" ) {
    ...
    REQUIRE(map.assign_territory_to_player(csguth_id, territory_id, 5));
...
TEST_CASE( "conquest territory from other player" ) {
    ...
    REQUIRE(map.assign_territory_to_player(other_id, territory_id, 1));
...
TEST_CASE( "only an empty territory can be dominated" ) {
    ...
    map.assign_territory_to_player(csguth_id, territory_id, 1);
    auto owner_before = map.owner(territory_id);
    REQUIRE(!map.assign_territory_to_player(other_id, territory_id, 1));
    REQUIRE(map.owner(territory_id) == owner_before);
}


TEST_CASE( "territory should be assigned to a valid player" ) {

    auto earth = create_earth();
    auto lobby = create_lobby();
    risk::domination::conquerable_map map(*earth, *lobby);
    risk::map::continent_territory_id territory_id(0, 0);
    player::id null_player = player::null_id();
    REQUIRE(!map.assign_territory_to_player(null_player, territory_id, 1));
    REQUIRE(map.owner(territory_id) == null_player);
}
