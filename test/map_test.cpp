...
TEST_CASE( "null territory object" ) {
    map themap("a");
    auto continent_id = themap.add_continent("america");
    REQUIRE(&themap.get_continent(continent_id).get_territory(1) == &territory::null);
}
...
TEST_CASE( "add territory" ) {
    ...
    REQUIRE(themap.get_continent(brazil_id.first).get_territory(brazil_id.second).name() == "brazil");
...
TEST_CASE( "connect territories" ) {
    ...
    REQUIRE(*themap.neighbors(brazil_id).first == colombia_id);
    REQUIRE(*themap.neighbors(colombia_id).first == brazil_id);
}
